...
def test_card_tier_table_ranks_exclusive_items_and_keeps_low_samples(tmp_path, monkeypatch):
    ...
    query.load()
    query.card_mapping = {card_id: {"name": card_id} for card_id in ("a", "b", "c", "low-sample", "shared")}
    query.card_heroes = {
...
def test_heroes_route_uses_the_runs_query_mapping_that_exists():
    ...
    assert "HERO_ZH_TO_EN" in route
    assert "HERO_EN_TO_ZH" not in route
